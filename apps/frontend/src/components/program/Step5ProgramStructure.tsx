...
export function Step5ProgramStructure({ formData, updateFormData, onNext, onBack }: Step5Props) {
  ...
  const getSessionsForBlockWeek = (blockId: string, absoluteWeekIndex: number) => {
    const blockSessions = formData.sessions.filter(s => s.blockId === blockId);
    const blockTimeline = timeline.find(t => t.blockId === blockId);
    if (!blockTimeline) return [];

    // Convert absolute week index to relative week within the block
    const relativeWeekIndex = absoluteWeekIndex - blockTimeline.startWeek;

    const scheduled = formData.scheduledSessions.filter(s =>
      s.blockId === blockId && s.startWeek === relativeWeekIndex
    );
